...
- В первой версии консультация по стоимости использует только марку, модель, год выпуска,
  приблизительный пробег, мощность двигателя и модель двигателя.
...
В CLI доступны `/car`, `/reset`, `/help`, `/exit` и сценарии `1`–`4`. Telegram
поддерживает `/start`, `/car`, `/help`, `/reset`, inline-выбор автомобиля и четыре
консультационных сценария без кнопок уточнения или обновления данных. Markdown в ответах
преобразуется в безопасный Telegram HTML; при ошибке разметки бот автоматически отправляет
plain text.
